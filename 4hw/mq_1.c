...
int main()
{
    ...
    msg_ptr = (struct my_message *)malloc(sizeof(struct my_message));

    msg_ptr->led_state = false;
    ...
    while(1)
    {
        ...
        strcpy(msg_ptr->buff, tmp_buff);

        memcpy(msg, &msg_ptr, sizeof(void *));
        ...
        if((mq_send(qd_proc2, msg, sizeof(void *), 0)) == -1)
        {
        ...
        if((mq_receive(qd_proc1, msg, 32, NULL)) == -1)
        {
        	perror("mq_receive\n");
        	exit(1);
        }

        memcpy(&tmp_ptr, msg, sizeof(void *));
